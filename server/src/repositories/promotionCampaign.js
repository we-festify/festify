...
class PromotionCampaignRepository {
  ...
  static async getAll({ extended = false }) {
    try {
      ...
      return await PromotionCampaign.find().select("-description -termsAndConditions");
    } catch (err) {
      throw err;
    }
  }
  ...
  static async updateById(id, promotionCampaign) {
    try {
      return await PromotionCampaign.findByIdAndUpdate(id, promotionCampaign, { new: true });
    } catch (err) {
      throw err;
    }
  }
